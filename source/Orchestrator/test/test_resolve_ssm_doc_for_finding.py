...
def test_sunny_day(mocker):
    ...
    ssmc_stub.activate()
    mocker.patch("resolve_ssm_doc_for_finding._get_ssm_client", return_value=ssm_c)
...
def test_doc_not_active(mocker):
    ...
    mocker.patch("resolve_ssm_doc_for_finding._get_ssm_client", return_value=ssm_c)
    mocker.patch(
        "resolve_ssm_doc_for_finding.CloudWatchMetrics.send_metric", return_value=None
    )
...
def test_alt_workflow_with_role(mocker):
    ...
    mocker.patch("resolve_ssm_doc_for_finding._get_ssm_client", return_value=ssm)
...
# ===============================================================================
# Multi-service finding tests (OCSF format)
# ===============================================================================


def test_multi_service_inspector_ocsf_finding(mocker):
    """Test that an OCSF Inspector finding with remediationId is handled correctly."""
    test_input = {
    ...
    expected_response = {
        ...
        "message": "",
        "remediation_status": "",
        # Inspector resolves to its dedicated, Inspector-only automation role
        # (not the shared Orchestrator-Member role) so the native patch waiter's
        # ssm:GetCommandInvocation grant stays confined.
        "remediationrole": "SO0111-ASR-Inspector-Automation",
        "resourceregion": "us-east-1",
    ...
    ssmc_stub.add_response(
        ...
        {
            "Document": {
                "Name": "ASR-Inspector.InstanceVulnerability",
                "Status": "Active",
                "DocumentType": "Automation",
            }
        },
...
def test_multi_service_guardduty_ocsf_finding(mocker):
    """Test that an OCSF GuardDuty finding with remediationId is handled correctly."""
    test_input = {
        "EventType": "Security Hub Findings - Imported",
        "Finding": {
            "class_uid": 2004,
            ...
            "resources": [
                {
                    "uid": "arn:aws:iam::222222222222:user/compromised-user",
                    "region": "eu-west-1",
...
def test_multi_service_iam_access_analyzer_asff_finding(mocker):
    """Test that an ASFF IAM Access Analyzer finding with remediationId is handled correctly."""
    test_input = {
        "EventType": "Security Hub Findings - Imported",
        "Finding": {
    ...
    expected_response = {
        ...
        "automationdocid": "ASR-IAMAccessAnalyzer.ExternalAccess",
    ...
    ssmc_stub.add_response(
        ...
        {
            "Document": {
                "Name": "ASR-IAMAccessAnalyzer.ExternalAccess",
                "Status": "Active",
                "DocumentType": "Automation",
            }
        },
...
def test_multi_service_doc_not_found(mocker):
    """Test that a multi-service finding with missing runbook returns NOTFOUND."""
    test_input = {
        "EventType": "Security Hub Findings - Imported",
        "Finding": {
            "class_uid": 2004,
            ...
            "resources": [
                {
                    "uid": "arn:aws:s3:::sensitive-bucket",
                    "region": "us-east-1",
                    "type": "AwsS3Object",
                }
            ],
    ...
    assert result["automationdocid"] == "ASR-Macie.SensitiveDataS3Object"
    assert result["controlid"] == "Macie.SensitiveDataS3Object"
    ssmc_stub.deactivate()


def test_existing_config_finding_still_works(mocker):
    """Test that existing Config/ASFF findings without remediationId still work (backward compatibility)."""
    test_input = {
        "EventType": "Security Hub Findings - Custom Action",
        "Finding": {
    ...
    result = lambda_handler(test_input, create_lambda_context())
    # Should follow the existing Config/ASFF path, not the multi-service path
    assert result["status"] == "ACTIVE"
    assert result["automationdocid"] == "ASR-AFSBP_1.0.0_AutoScaling.1"
    assert result["securitystandard"] == "AFSBP"
    ssmc_stub.deactivate()


def test_no_detail_metadata_defaults_to_asff(mocker):
    """Test backward compatibility when FindingFormat/RemediationId are not present (e.g., Custom Action from UI)."""
    test_input = {
        "EventType": "Security Hub Findings - Custom Action",
        "Finding": {
            "Id": "arn:aws:securityhub:us-east-1:111111111111:subscription/aws-foundational-security-best-practices/v/1.0.0/AutoScaling.1/finding/test",
        ...
        # No FindingFormat/RemediationId — simulates Custom Action from UI
    }
...
def test_multi_service_ocsf_missing_account_id():
    """Test that an OCSF finding missing cloud.account.uid returns ERROR."""
    test_input = {
        "EventType": "Security Hub Findings - Imported",
        "Finding": {
            "class_uid": 2002,
    ...
    result = lambda_handler(test_input, create_lambda_context())
    assert result["status"] == "ERROR"
    assert "Could not determine account ID" in result["message"]


def test_multi_service_invalid_remediation_id():
    """Test that a malformed remediation_id (e.g. path traversal) returns ERROR."""
    test_input = {
        "EventType": "Security Hub Findings - Imported",
        "Finding": {
            "class_uid": 2002,
    ...
    result = lambda_handler(test_input, create_lambda_context())
    assert result["status"] == "ERROR"
    assert "Invalid remediation ID format" in result["message"]

...
from check_ssm_execution import lambda_handler, AutomationExecution
...
def test_failed_remediation(mocker):
    ...
    ssmc_stub.add_response(
        ...
        ssm_mocked_failed_response,
        {'Filters': [{'Key': 'ExecutionId', 'Values': ['43374019-a309-4627-b8a2-c641e0140262']}]}
    )
    ssmc_stub.activate()
...
def test_execid_parsing_nonsharr(mocker):
    ...
    automation_exec_info = AutomationExecution(
        test_event['SSMExecution']['ExecId'],
        account,
        'foo-bar-baz',
        'us-east-1'
        )
    assert automation_exec_info.status == 'Success'
    assert automation_exec_info.outputs == ssm_mocked_good_response['AutomationExecutionMetadataList'][0]['Outputs']
...
def test_execid_parsing_sharr(mocker):
    ...
    automation_exec_info = {
        "AutomationExecutionId": "795cf453-c41a-48df-aace-fd68fdace188",
        "DocumentName": "SHARR_Remediation_AFSBP_1.0.0_EC2.7",
        "DocumentVersion": "1",
        "AutomationExecutionStatus": "Success",
        "ExecutionStartTime": "2021-05-06T15:16:06.718000-04:00",
        "ExecutionEndTime": "2021-05-06T15:16:37.083000-04:00",
        "ExecutedBy": "arn:aws:sts::111111111111:assumed-role/SO0111-SHARR-Orchestrator-Member_us-east-1/sechub_admin",
        "LogFile": "",
        "Outputs": {
            "ParseInput.AffectedObject": [
                "{\"Type\":\"EBSEncryption\",\"Id\":\"EBSEncryptionByDefault\",\"OutputKey\":\"Remediation.Output\"}"
            ],
            "Remediation.Output": [
                "{\"message\":\"Encryption by default enabled on EBS\",\"status\":\"Success\"}"
            ]
        },
        "Mode": "Auto",
        "Targets": [],
    ...
    expected_result = {
        'affected_object': 'UNKNOWN',
        'executionid': '795cf453-c41a-48df-aace-fd68fdace188',
        'logdata': '[]',
        'message': '{"ModifyAccount.EnableEbsEncryptionByDefaultResponse": ["{\\"EbsEncryptionByDefault\\":true,\\"ResponseMetadata\\":{\\"RequestId\\":\\"c45a9839-5a40-472e-ac83-d0058987948c\\",\\"HTTPStatusCode\\":200,\\"HTTPHeaders\\":{\\"x-amzn-requestid\\":\\"c45a9839-5a40-472e-ac83-d0058987948c\\",\\"cache-control\\":\\"no-cache, no-store\\",\\"strict-transport-security\\":\\"max-age\\\\u003d31536000; includeSubDomains\\",\\"content-type\\":\\"text/xml;charset\\\\u003dUTF-8\\",\\"transfer-encoding\\":\\"chunked\\",\\"vary\\":\\"accept-encoding\\",\\"date\\":\\"Thu, 06 May 2021 19:16:14 GMT\\",\\"server\\":\\"AmazonEC2\\"},\\"RetryAttempts\\":0}}"]}',
        'remediation_status': 'Success',
        'status': 'Success'
    ...
    ssmc_stub.add_response(
        'describe_automation_executions',
        ssm_mocked_good_response,
        {'Filters': [{'Key': 'ExecutionId', 'Values': ['795cf453-c41a-48df-aace-fd68fdace188']}]}
    )
    ssmc_stub.activate()

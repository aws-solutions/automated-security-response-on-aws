...
def expected():
    return {
        ...
        'details':  {'AwsS3Bucket': {'CreatedAt': '2022-10-27T20:32:54.000Z','OwnerId': '93b93c44fd03f06ac297d9923da9bf86507301cfc9485b1d29c992241afd5182'}},
...
def test_control_id_nomatch(mocker):
    ...
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        parsed_event = parse_event(test_event, {})
    assert pytest_wrapped_e.type == SystemExit
    assert pytest_wrapped_e.value.code == 'ERROR: Control Id from input (2.4) does not match S3.12'
...
def test_bad_resource_match(mocker):
    ...
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        parsed_event = parse_event(test_event, {})
    assert pytest_wrapped_e.type == SystemExit
    assert pytest_wrapped_e.value.code == 'ERROR: Invalid resource Id arn:aws:s3:::asr-scv-reference'

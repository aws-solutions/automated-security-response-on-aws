...
def test_EnableVPCFlowLogs_success(mocker):
    event = {
        ...
        'region': my_region,
        'retries': 1, 'wait': 1  # for testing, so not waiting 60 seconds for a stub
    }

    log_group_name = 'VPCFlowLogs/' + event['vpc']
    ...
    ### LOGS
    logs_client = botocore.session.get_session().create_client('logs', config=BOTO_CONFIG)
    ...
    ec2_stubber.add_response(
        ...
        {},
        {
            ...
            'LogGroupName': 'VPCFlowLogs/' + event['vpc'],
...
def test_EnableVPCFlowLogs_loggroup_fails(mocker):
    ...
    event = {
        ...
        'kms_key_arn': 'arn:aws:kms:us-west-2:111111111111:key/1234abcd-12ab-34cd-56ef-1234567890ab',
        'region': my_region,
        'retries': retries, 'wait': 1  # for testing, so not waiting 60 seconds for a stub
    }

...
def test_success(mocker):
    ...
    iam_stubber.add_response(
        'attach_role_policy',
        {},
        test_case.attach_params()
    )

    iam_stubber.activate()

    mocker.patch('ReplaceCodeBuildClearTextCredentials.connect_to_ssm', return_value = ssm_client)
    mocker.patch('ReplaceCodeBuildClearTextCredentials.connect_to_iam', return_value = iam_client)
...
def test_policy_exists(mocker):
    ...
    successful_response = {
        ...
        'Policy': {
            'Policy': {
                'Arn': test_case.policy_serialized()['Policy']['Arn']
            }
        },
        'UpdatedProjectEnv': project_env
    }
...
def test_put_parameter_fails(mocker):
    ...
    mocker.patch('ReplaceCodeBuildClearTextCredentials.connect_to_ssm', return_value = ssm_client)
    mocker.patch('ReplaceCodeBuildClearTextCredentials.connect_to_iam', return_value = None)

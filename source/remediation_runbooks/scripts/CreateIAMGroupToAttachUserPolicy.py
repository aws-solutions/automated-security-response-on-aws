...
def create_iam_group_to_attach_user_policy(event, context):
    ...
    inline_policies = (
        aws_iam_user["UserPolicyList"] if "UserPolicyList" in aws_iam_user else []
    )
    ...
    if inline_policies:
        for policy in inline_policies:
            ...
            policy_details = iam.get_user_policy(
                UserName=username, PolicyName=current_policy_name
            )
            ...
            if (
                attach_policy_response["ResponseMetadata"]
                and attach_policy_response["ResponseMetadata"]["HTTPStatusCode"]
                and attach_policy_response["ResponseMetadata"]["HTTPStatusCode"] == 200
            ):
    ...
    response = iam.add_user_to_group(GroupName=group_name, UserName=username)

    responses["CreateIAMGroupToAttachUserPolicyResponse"].append(
        {"AwsIamUser": aws_iam_user, "Response": response}
    )

    return {"output": "IAM Group creation is successful.", "http_responses": responses}

...
# Normalized status values published as metrics. Fixed set, so a Literal lets
# the type checker verify the mapping in get_status_for_metrics is exhaustive.
...
# Remediation identifiers for remediations that were generated by an AI agent.
#
# DO NOT EDIT THIS SET BY HAND. It is maintained exclusively by AI agents that
# create custom remediations (see ai-assets/prompts/remediation-generator-system-prompt.md).
# Internal builders adding human-authored remediations must NOT add entries here.
#
# Each entry is the remediation identifier as registered in the playbook
# remediation list (e.g. the `control` value in sc_remediations.ts). For Security
# Hub control findings this is the control ID (e.g. "S3.14"); for non-control
# findings it is whatever identifier the remediation was registered under.
#
# When a finding's remediation identifier is present in this set, metrics
# published to the Solutions metrics endpoint are tagged with `ai_generated: true`
# so AI-generated remediations can be distinguished from human-authored ones.
AI_GENERATED_REMEDIATION_IDS: frozenset[str] = frozenset()


def is_ai_generated_remediation(remediation_id: str) -> bool:
    """Return True if the remediation identifier was generated by an AI agent."""
    return remediation_id in AI_GENERATED_REMEDIATION_IDS
...
class Metrics(object):
    ...
    @staticmethod
    def get_status_for_metrics(status_from_event: str) -> Tuple[MetricStatus, str]:
        """
        ...
        if status_from_event_upper == "SUCCESS":
            ...
            status = "PENDING"
        elif status_from_event_upper == "ROLLBACK_IN_PROGRESS":
            status = "ROLLBACK_IN_PROGRESS"
        elif status_from_event_upper == "ROLLBACK_SUCCESS":
            status = "ROLLBACK_SUCCESS"
        elif status_from_event_upper == "ROLLBACK_FAILED":

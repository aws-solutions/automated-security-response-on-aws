...
from simtest.orchestrator import get_orchestrator, get_session
import sys
import json

sys.path.append('../source/LambdaLayers')
...
class ControlTest:
    ...
    def print_heading(self, description):
        print('=' * 80)
        print(f'Simulate {self.finding.standard_shortname} {self.finding.standard_control} Findings\n')
...
class RemediationTest:
    def __init__(self, remediation, account, wrap_it_in_findings = False):
        ...
        self.test_json['detail']['findings'][0]['Resources'][0]['Region'] = self.orchestrator.get_region()
    ...
    def _substitute_config_rules(self):
        ...
        while finding.get('ProductFields', {}).get(type_key, None):
            if (finding['ProductFields'][type_key] == 'AWS::Config::ConfigRule'):
                ...
                rule_name_prefix = rule_name[0:len(rule_name) - len(rule_name.split('-')[-1])]
                finding['ProductFields'][name_key] = rule_store.get_rule_name_from_prefix(rule_name_prefix)
...
def wrap_in_findings(test_json):
    wrapper = {
        ...
        "region": "us-east-2",
        "resources": [
            "arn:aws:securityhub:us-east-2:111111111111:action/custom/foo"
        ],
        "detail": {
            "actionName": "foo",

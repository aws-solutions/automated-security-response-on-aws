...
def run_setup_config(remediation, account, region):
    ...
    print(f'This test enables AWS Config\n')
    ...
    if account == aws.get_account():
        ...
        print(f'3) Remove only the config bucket, so0111-aws-config-{region}-{account}. This tests that the remediation can use the existing access logging bucket, if you ran CloudTrail.1 before Config.1')
        print('Note: Step 3 may take a while')
        input('HIT ENTER TO START')

        delete_default_config_recorder()
        delete_default_delivery_channel()
        delete_sns_topic('SO0111-SHARR-AFSBP-Config-1-AWSConfigNotification', account, region)
        ...
        print('2) Remove SNS Topic SO0111-SHARR-AFSBP-Config-1-AWSConfigNotification')
        print(f'3) Remove the config bucket, so0111-aws-config-{region}-{account}. This tests that the remediation can use the existing access logging bucket, if you ran CloudTrail.1 before Config.1')
        print('Note: Step 3 may take a while')
    ...
    print('\nVERIFICATION\n============\n')
    print(f'1) Config is enabled for all resources, globally, with logging to so0111-aws-config-{region}-{account}')
    print(f'2) Bucket so0111-aws-config-{region}-{account} is encrypted and has access logging enabled')
    print(f'3) Config data is flowing to the config bucket, so0111-aws-config-{region}-{account}')

def delete_default_config_recorder():
    aws = get_session()
    cfgsvc = aws.client('config')
    try:
        cfgsvc.delete_configuration_recorder(
        ...
        if exception_type == "NoSuchConfigurationRecorderException":
            print(f'Default delivery recorder does not exist...continuing')
        else:
...
def delete_default_delivery_channel():
    ...
    cfgsvc = aws.client('config')
    try:
        cfgsvc.delete_delivery_channel(
        ...
        if exception_type == "NoSuchDeliveryChannelException":
            print(f'Default delivery channel does not exist...continuing')
        else:
            print(f'Unhandled client error {exception_type} deleting default delivery channel')
            raise
    except Exception as e:
        print(e)
        print('Something went wrong')
...
def delete_sns_topic(topicname, account, region):
    ...
    try:
        ...
        if exception_type == "NotFoundException":
            ...
            print(f'Unhandled client error {exception_type} deleting sns topic {topicname}')
            raise
    except Exception as e:
        print(e)
        print('Something went wrong')

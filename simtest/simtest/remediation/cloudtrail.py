...
from simtest.remediation_test import RemediationTest, ControlTest
...
def run_create_multi_region_cloudtrail(remediation, account, region):
    ...
    remtest.test_json['detail']['findings'][0]['Resources'][0]['Id'] = 'AWS::::Account:' + account
    ...
    remtest.run()
    remtest.print_verification_instructions('Verify that a multi-region cloudtrail was created with S3 bucket')
...
def run_enable_cloudtrail_logfile_validation(remediation, account, region):
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = 'arn:aws:cloudtrail:' + \
        region + ':111111111111:trail/' + trail_name
...
def run_make_cloudtrail_s3_bucket_private(remediation, account, region):
    ...
    print('SETUP\n=====\n')
    print('1) Create an S3 bucket in the same region as the test. DO NOT MAKE IT PUBLIC!')
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = 'arn:aws:s3:::' + bucket_name
...
def run_create_ct_access_logging(remediation, account, region):
    ...
    print('\nVerify that S3 bucket so0111-sharr-cloudtrailaccesslogs-<account>-' + region + ' was created in the target account. If the bucket already existed then simply check the lambda logs for errors.')

def run_enable_ct_encryption(remediation, account, region):
    aws = get_session()
    print(f'This test enables encryption on a CloudTrail\n')
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = f'arn:{aws.get_partition()}:cloudtrail:{region}:{account}:trail/{cloudtrail}'
    test.test_json['detail']['findings'][0]['Resources'][0]['Details']['AwsCloudTrailTrail']['HomeRegion'] = region
...
def run_create_cloudtrail_multi_region_trail(remediation, account, region):
    aws = get_session()
    print('Simulate AFSBP CloudTrail.1 Findings\n')
    ...
    if account == aws.get_account():
        print('Automatic Setup\n')
        print('===============\n')
        print('1) Remove the CloudTrail named "multi-region-cloud-trail", if it exists already.')
        print(f'2) Remove the two buckets, so0111-access-logs-{region}-{account} and so0111-aws-cloudtrail-{account}')
    ...
    print('\nVERIFICATION\n============\n')
    print('1) CloudTrail "multi-region-cloud-trail" was created, is encrypted, and is enabled for all regions')
    print(f'2) Buckets so0111-access-logs-{region}-{account} and so0111-aws-cloudtrail-{account} were created')
    print(f'3) CloudTrail log data is flowing to the so0111-aws-cloudtrail-{account} bucket')
    print(f'4) Access logs for so0111-aws-cloudtrail-{account} bucket are delivered to so0111-access-logs-{region}-{account} bucket')
...
def delete_cloudtrail(trailname):
    ...
    try:
        ...
        print(f'Deleted CloudTrail multi-region-cloud-trail')
...
def delete_bucket(bucketname):
    ...
    try:
        ...
        if exception_type in "NoSuchBucket":
            ...
            print(f'Unhandled client error {exception_type} deleting bucket {bucketname}')
            raise
    except Exception as e:
        print(e)
        print('Something went wrong')

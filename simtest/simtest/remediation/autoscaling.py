...
def run_autoscaling_1(remediation, account, region):
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = f'arn:aws:autoscaling:{region}:{account}:autoScalingGroup:785df3481e1-cd66-435d-96de-d6ed5416defd:autoScalingGroupName/{asg_name}'

    test.run()

    print('\nVERIFICATION\n============\n')
    print(f'1) Verify that ELB health check is enabled on the autoscaling group.')

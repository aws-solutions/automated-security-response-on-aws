...
def run_remove_public_ec2_snaps(remediation, account, region):
    ...
    if account == aws.get_account():
        ...
        print('===============\n')
        print('1) Select a snapshot from your test account. Snapshot must not have sensitive, customer, or production data!')
...
def run_close_default_sg(remediation, account, region):
    ...
    print('This test removes open ports in- and outbound from the security group in the finding. You will need to create a security group in the AWS console.\n')

    print('Manual Setup\n')
    print('============\n')
    print(f'Please do the following')
    print('1) Create (or select) a default Security Group in your test account. It must be named "default". Allow all inbound and outbound traffic from/to anywhere. Enter the Security Group Id below.')
    sg_id = input('Security Group Id?: ')

    test = RemediationTest(remediation, account, wrap_it_in_findings=True)

    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = f'arn:aws:ec2:{region}:{account}:security-group/{sg_id}'
    test.test_json['detail']['findings'][0]['Resources'][0]['Details']['AwsEc2SecurityGroup']['GroupId'] = sg_id
...
def run_disable_public_access_for_security_group(remediation, account, region):
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Id'] = "arn:aws:ec2:us-east-2:111111111111:security-group/" + sg_id
    test.test_json['detail']['findings'][0]['Resources'][0]['Details']['AwsEc2SecurityGroup']['GroupId'] = sg_id
    test.run()
...
def run_remove_vpc_default_security_group_rules(remediation, account, region):
    ...
    print('SETUP\n=====\n')
    print('1) Create a Security Group')
    print('2) Configure it to allow open inbound and outbound access: all protocols from/to anywhere.\n')
...
def run_enable_ebs_encryption_by_default(remediation, account, region):
    aws = get_session()
    print('Simulate AFSBP EC2.7 Findings\n')
    ...
    print('\nVERIFICATION\n============\n')
    print(f'1) In EC2 Dashboard, click EBS Encryption under Account Attributes and confirm that it is enabled.')

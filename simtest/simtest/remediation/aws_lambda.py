...
def run_make_lambda_private(remediation, account, region):
    ...
    if account == aws.get_account():
        ...
        print('===============\n')
        print('1) Create a lambda in the test account using the "Hello, world!" example code.')
        ...
        print('===============\n')
        print('1) Create a lambda in the test account using the "Hello, world!" example code.')
        print('2) Make the lambda public by running the following CLI in the target account:')
        print('  aws lambda add-permission --function-name <name> --statement-id SHARRTest --action lambda:InvokeFunction --principal \'*\'')
    ...
    test.test_json['detail']['findings'][0]['Resources'][0]['Details']['AwsLambdaFunction']['FunctionName'] = lambda_name
    test.run()
